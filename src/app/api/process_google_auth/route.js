import { googleAuth } from "@/app/api/service";
import { cookies } from "next/headers";
export async function POST(request) {
  const res = await request.json();
  const googleAuthUrl = res.post;
  const loggedUserData = await googleAuth(googleAuthUrl);
  ...
  const class_id = loggedUserData.user.class_id;
  const user_coin = loggedUserData.user_coin;
  const userClassName = loggedUserData.user.class_name;
  const coinScore =
  ...
  return Response.json(
    // "SuccessFully Login"
    {
      ...
      class_id: loggedUserData.user.class_id,
    }
    //   , {
    //   status: 200,
    //   headers: {
